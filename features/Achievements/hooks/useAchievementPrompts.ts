import { useStatsStore, type StatsStoreState } from '@/features/Progress';
...
export const useAchievementPrompts = (): UseAchievementPromptsReturn => {
  ...
  const getRelevantAchievements = useCallback(
    (contentType: string): Achievement[] => {
      const allAchievements = achievementStore.getAchievementsByCategory(
        contentType === 'hiragana' || contentType === 'katakana'
          ? 'kana'
          : contentType === 'kanji'
            ? 'kanji'
      ...
      const streakAchievements =
        achievementStore.getAchievementsByCategory('streak');
      const milestoneAchievements =
        achievementStore.getAchievementsByCategory('milestone');
      ...
      return [
        ...
        achievement => !achievementStore.unlockedAchievements[achievement.id],
      );
    },
    [achievementStore],
  );
  ...
  const calculateProgress = useCallback(
    (achievement: Achievement, stats: StatsStoreState): number => {
      ...
      switch (type) {
        ...
        case 'content_correct':
          if (additional?.contentType === 'hiragana') {
            return Math.min(
              (stats.allTimeStats.hiraganaCorrect / value) * 100,
              100,
            );
          } else if (additional?.contentType === 'katakana') {
            return Math.min(
              (stats.allTimeStats.katakanaCorrect / value) * 100,
              100,
            );
          } else if (additional?.contentType === 'kanji') {
            const jlptLevel = additional?.jlptLevel;
            const kanjiCorrect = jlptLevel
              ? (stats.allTimeStats.kanjiCorrectByLevel[jlptLevel] ?? 0)
              : 0;
            return Math.min((kanjiCorrect / value) * 100, 100);
          } else if (additional?.contentType === 'vocabulary') {
            return Math.min(
              (stats.allTimeStats.vocabularyCorrect / value) * 100,
              100,
            );
          }
          return 0;
  ...
  const generateContext = useCallback(
    (achievement: Achievement, progress: number): string => {
      ...
      switch (type) {
        ...
        case 'content_correct':
          if (achievement.requirements.additional?.contentType === 'hiragana') {
            ...
            achievement.requirements.additional?.contentType === 'katakana'
          ) {
  ...
  const checkForAchievementProgress = useCallback(
